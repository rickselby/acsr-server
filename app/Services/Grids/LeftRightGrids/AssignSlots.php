...
class AssignSlots
{
     ...
     * @param int $driversPerHeat   Number of drivers per heat
     * @param $heatsPerDriver       Number of heats each driver participates in
     *
     * @return array
     */
    public function assign($driverCount, $driversPerHeat, $heatsPerDriver)
    {
        // Round drivers up to nearest even number
        $driverCount += ($driverCount % 2);

        // Generate the array framework to work with
        $this->generateFramework($driverCount, $driversPerHeat, $heatsPerDriver);
     ...
     * @return array
     */
    protected function generateFramework($driverCount, $driversPerHeat, $heatsPerDriver)
    {
        // How many 'slots' per heat are there?
        $heatSlots = (int) $driversPerHeat / 2;

        // How many heats do we need?
        $heatCount = (int) ($driverCount * $heatsPerDriver) / $driversPerHeat;
